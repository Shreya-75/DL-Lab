...
Y = torch.tensor([[0], [1], [1], [0]], dtype=torch.float32)  # Reshape Y to be a 2D tensor
...
class XORModel(nn.Module):
    def __init__(self):
        ...
        self.activation2 = nn.Sigmoid()  # Use Sigmoid for the output layer
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):  # Fixed method name from __getitem to __getitem__
...
with torch.no_grad():
    ...
    output = output.cpu().numpy()# Add batch dimension
    if(output>0.5):
        output = 1
    else:
        output=0
